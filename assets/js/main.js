(async function () {
  ...
  window.addEventListener("load", async () => {
    ...
    portfolioContainer.innerHTML =
      await fetch("https://api.github.com/search/repositories?q=user:diasutsman%20topic:project&per_page=100")
        .then((response) => response.json())
        .then((data) => data.items
          .filter(({ topics }) => topics.find(topic => ['web', 'android', 'back-end', 'flutter'].includes(topic)))
          .map((portfolio) => ({
            ...portfolio,
            category: portfolio.topics.find(topic => ['web', 'android', 'back-end', 'flutter'].includes(topic)),
            name: normalizeText.normalizeName(portfolio.name.replace(/[-_]/g, ' ').replace(/[a-z][A-Z]/g, c => `${c[0]} ${c[1]}`)),
          }))
          .map((portfolio) => portfolioTemplate(portfolio)).join("")
        );
