const mobilApp = new Swiper('.swiper', {
    // Optional parameters
...
certsImages.forEach((certsImage) => {
    certsSlide.appendSlide(`
      <div class="swiper-slide">
          <img src="${certsImage}" class="img-fluid rounded-4" alt="certs-image">
      </div>
  `);
})
...
skillsContent.innerHTML = skills.map(skill => {
    return `<div class="flip-card bg-transparent">
    <div class="flip-card-inner position-relative w-100 h-100 text-center">
      <div class="flip-card-front position-absolute h-100 w-100 d-flex justify-content-center align-items-center flex-column gap-3">
        <img class="w-50" src="${skill.image}" alt="${skill.name}">
        <p>${skill.name}</p>
      </div>
      <div class="flip-card-back position-absolute h-100 w-100 d-flex justify-content-center align-items-center flex-column gap-3">
        <h1 class="mb-0">${skill.name}</h1>
        <p>${skill.desc}</p>
        <div class="percentage w-75 rounded-4 overflow-hidden d-flex align-items-center gap-3">
          <div class="progress h-100" style="width: ${skill.percentage}%"></div>
        </div>
      </div>
    </div>
  </div>`
